...
from .journey import build_journey_graph
from .metrics import MetricsEngine, Turn
...
def _build_case_payload(
    *,
    ...
    layered_metrics = _build_layered_metrics(
        events=events,
...
def _build_layered_metrics(
    *,
    ...
    grouped_tools: dict[int, dict[str, set[str]]] = {}
    ...
    for event in events:
        ...
        if not isinstance(path, str) or not path or path == "." or path.endswith("/"):
            continue
        grouped_files.setdefault(turn, set()).add(path)
        if isinstance(tool_name, str) and tool_name:
            grouped_tools.setdefault(turn, {}).setdefault(tool_name, set()).add(path)

    turns = [
        Turn(
            index=turn,
            tool_calls=[
                {"tool": tool_name, "files": sorted(paths)}
                for tool_name, paths in sorted(grouped_tools.get(turn, {}).items())
            ],
            files_accessed=sorted(grouped_files[turn]),
        )
        for turn in sorted(grouped_files)
    ]
