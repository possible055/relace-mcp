...
- **Pluggable code indexing backends** — `MCP_RETRIEVAL_BACKEND` supports `relace`, `codanna`, `chunkhound`, or `none` for flexible semantic search.
- **`MCP_LOG_LEVEL=OFF`** — Completely disable logging for clean stdio transport.

### Fixed

- **Windows stdio transport** — Fixed JSON-RPC protocol corruption caused by CRLF line endings.
- **API error handling** — Improved categorization for rate limits (429) and server errors (5xx).

### Changed

- **`MCP_LOGGING` unified** — Now accepts `off`, `safe`, or `full`. Replaces `MCP_LOG_REDACT`.
- **`MCP_SEARCH_MODE` → `MCP_SEARCH_RETRIEVAL`** — Simplified to boolean flag.

### Removed

- **`fast_search` tool** — Use `agentic_search` instead.
- **Deprecated `RELACE_*` environment variables** — All removed. Use new names (`APPLY_*`, `SEARCH_*`, `MCP_*`).
