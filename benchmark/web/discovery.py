...
from benchmark.experiments.layout import SEARCH_MAP_BUNDLE_FILENAME, SUMMARY_FILENAME

REPORT_FILENAME = SUMMARY_FILENAME
...
def _bundle_case_count(experiment_root: Path) -> int | None:
    bundle_path = experiment_root / "analysis" / SEARCH_MAP_BUNDLE_FILENAME
...
def _summary_from_report(report_path: Path) -> dict[str, Any]:
    ...
    summary: dict[str, Any] = {
        ...
        "has_bundle": (experiment_root / "analysis" / SEARCH_MAP_BUNDLE_FILENAME).exists(),
...
def list_experiments(experiments_root: Path) -> list[dict[str, Any]]:
    ...
    for bundle_path in sorted(experiments_root.rglob(SEARCH_MAP_BUNDLE_FILENAME)):
        experiment_root = (
            ...
            if bundle_path.parent.name == "analysis"
