...
from relace_mcp.apply import apply_file_logic
from relace_mcp.apply.logging import log_event
...
class TestApplyFileLogicRecoverableErrors:
    ...
    async def test_anchor_precheck_skipped_with_append_directive(
        ...
        """Append-like instructions should still require anchors (no instruction-based skip)."""
        test_file = tmp_path / "test.py"
    ...
    async def test_permission_error_returns_permission_error(
        ...
        with patch(
            "relace_mcp.apply.core.read_text_with_fallback",
    ...
    async def test_filesystem_error_returns_fs_error_on_create(
        ...
        with patch(
            "relace_mcp.apply.core.atomic_write",
...
class TestApplyNoopDetection:
    ...
    @pytest.mark.asyncio
    async def test_noop_with_omission_style_context_returns_ok(
        self,
        _mock_config: RelaceConfig,
        mock_backend: AsyncMock,
        tmp_path: Path,
    ) -> None:
        """Context-only omission syntax should not force APPLY_NOOP when merge is idempotent."""
        test_file = tmp_path / "test.py"
        original_content = (
        ...
        result = await apply_file_logic(
            ...
            instruction="Keep this block intact",
            base_dir=str(tmp_path),
        )
